...
class PoseEstPostProcessing:
    ...
    def visualize_pose_estimation_result(
        self, results: dict, img: Image.Image, *, 
        detection_threshold: float = 0.3,
        joint_threshold: float = 0.3
    ) -> np.ndarray:
        """Modified visualization that removes padding before display"""
        ...
        # Get original image dimensions (without padding)
        h, w = image.shape[:2]
        ...
        if 'predictions' in results:
            ...
            bboxes, scores, keypoints, joint_scores = (
                ...
                results['keypoints'], results['joint_scores']
            )
        ...
        for (detection_box, detection_score, detection_keypoints,
            ...
            for joint, joint_score in zip(detection_keypoints, detection_keypoints_score):
                if joint_score < joint_threshold:
                    continue
                cv2.circle(image, (int(joint[0]), int(joint[1])), 3, (255, 0, 255), -1)
